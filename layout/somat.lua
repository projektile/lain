...
--    useless_gap = tonumber(beautiful.useless_gap_width) or 0
...
--    bw = tonumber(beautiful.border_width) or 0
    ...
    size = math.min(size, (available - (global_border/2)))
    local coord = wa[y]
    local geom = {}
    local used_size = 0
    local unused = wa[height] - (global_border / 2) --good but needs counterbalance         -- testing here HIGHLIGHT
    local stat_coord = wa[x]
    --stat_coord = size
    for c = group.first,group.last do
        local i = c - group.first +1
        geom[width] = size - (global_border/2) - useless_gap --+ (bw * 2))                -- and here
        geom[height] = math.floor(unused * fact[i] / total_fact) - useless_gap/2
        geom[x] = group.coord + (useless_gap / 1) -- + global_border
        geom[y] = coord + (useless_gap / 1) -- + global_border
        ...
        if useless_gap > 0
        then
            -- Top and left clients are shrinked by double size
            -- of the useless gap, all others, only the standard
...
local function tile(param, orientation)
    ...
    wa.height = wa.height - (global_border * 2) + (bw * 2) --may need to be * 2
    wa.width = wa.width - (global_border * 1) + (bw * 2)  -- PRETTY SURE THE *1 IS ACCURATE AND NEEDS TO BE COUNTERBALANCED ELSEWHERE
    ...
    for d = 1,2 do
        if place_master and nmaster > 0 then
            local size = wa[width]
            if nother > 0 then
                size = math.min(wa[width] * mwfact, wa[width] - (coord - wa[x])) + (global_border / 2)     -- add back here if issue
            end
            if not data[0] then
                data[0] = {}
            end
        ...
        if not place_master and nother > 0 then
            ...
            local wasize = wa[width] - global_border*2 --      i see no results from this everytime      OR HERE
            if nmaster > 0 and (orientation == "left" or orientation == "top") then
                wasize = wa[width] - wa[width]*mwfact + global_border/2
            end
            for i = 1,ncol do
